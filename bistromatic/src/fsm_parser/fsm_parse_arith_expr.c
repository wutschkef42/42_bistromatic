...
static int bsm_init_state_machine(t_fsm **fsm, int expr_len, t_position *pos)
{
    if (!((*fsm) = (t_fsm*)malloc(sizeof(t_fsm))) ||
       !((*fsm)->rpn_arith_expr = (char*)malloc(sizeof(char) * expr_len * 2 + 10)))
    {
...
static void	bsm_pop_operator_stack(t_fsm *fsm, char *rpn_arith_expr, t_position *pos, t_stack **operators)
{
...
{
    state_table[fsm->current_state][3](fsm, -1);
    if (fsm->current_state == STATE_ParseError)
        return (NULL);
    bsm_pop_operator_stack(fsm, fsm->rpn_arith_expr, fsm->pos, &(fsm->operators));
    if (fsm->current_state == STATE_ParseError)
        return (NULL);
...
char	*bsm_infix_to_rpn(const char *infix_arith_expr, int expr_len)
{
            ...
            fsm->pos->infix_pos < expr_len)
    {
        ...
        state_table[fsm->current_state][fsm->current_event](fsm, c);
    }
